...
import { ADD_NEW_CATEGORY, GET_ALL_CATEGORIES } from "../types/categoryType";
import {
  requestFailed,
  requestStarted,
  requestSucceeded,
...
export const rquestCreateCategory = (data) =>{
    return async (dispatch,  getState) =>{
        ...
        dispatch(requestStarted())
        try {
            const res = await axios.post(`${process.env.REACT_APP_API_URL}/category/add-new-category`, data,{header:{authorization:token}})
            dispatch(requestSucceeded());
